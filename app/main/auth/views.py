...
def logout():
    logout_user()
    flash(u'Anda telah berhasil keluar!', 'info')
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'Pendaftaran berhasil! Selamat Datang %s!' % form.name.data, 'success')
        login_user(the_user)
        return redirect(request.args.get('next') or url_for('main.index'))
    return render_template('register.html', form=form, title=u"Pendaftaran anggota baru")
...
def change_password():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'Kata sandi berhasil diperbarui!', 'info')
        return redirect(url_for('user.detail', user_id=current_user.id))
    return render_template('user_edit.html', form=form, user=current_user, title=u"Ganti kata sandi")
