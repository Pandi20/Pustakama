...
def edit(book_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(book)
        db.session.commit()
        flash(u'Informasi buku telah disimpan!', 'success')
    ...
    return render_template("book_edit.html", form=form, book=book, title=u"Edit informasi buku")
...
def add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'%s buku telah ditambahkan ke Pustakama!' % new_book.title, 'success')
        return redirect(url_for('book.detail', book_id=new_book.id))
    return render_template("book_edit.html", form=form, title=u"Tambahkan buku baru")
...
def delete(book_id):
    ...
    db.session.commit()
    flash(u'Buku berhasil dihapus, anggota tidak dapat lagi melihat buku tersebut.', 'info')
...
def put_back(book_id):
    ...
    db.session.add(the_book)
    db.session.commit()
    flash(u'Buku berhasil dipulihkan, anggota sekarang dapat melihat buku tersebut', 'info')
    return redirect(request.args.get('next') or url_for('book.detail', book_id=book_id))
